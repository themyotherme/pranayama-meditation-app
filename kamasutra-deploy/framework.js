...
class WellnessFramework {
    ...
    async loadConfig() {
        try {
            const response = await fetch('examples/kamasutra-app-config.json');
    ...
    updateUI() {
        ...
        if (programsSubtitle && this.config?.ui?.programsSubtitle) {
            programsSubtitle.innerHTML = `${this.config.ui.programsSubtitle} <button onclick="framework.showAIPrograms()" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; padding: 0.4rem 0.8rem; border-radius: 6px; cursor: pointer; font-weight: 600; text-decoration: none; display: inline-block; margin: 0 0.3rem; font-size: 0.875rem;">${this.config.ui.aiProgramsButton || '🤖 Your AI Programs'}</button>`;
        }
    ...
    async loadExercises() {
        try {
            ...
            const response = await fetch('examples/kamasutra-exercises-complete.json?v=' + Date.now() + '&cache=' + Math.random());
    ...
    async loadPrograms() {
        try {
            ...
            const response = await fetch('examples/kamasutra-programs-complete.json?v=' + Date.now() + '&cache=' + Math.random());
            const data = await response.json();
        ...
        card.addEventListener('click', (e) => {
            if (clickTimer) {
                ...
                clickTimer = setTimeout(() => {
                    clickTimer = null;
                    // Confirmed single click - show details
            ...
            <div id="ai-programs-panel" class="ai-programs-panel" style="display: block;">
                ...
                <div class="ai-programs-content">
                    <div class="ai-programs-actions">
                        <button class="btn btn-primary" onclick="window.framework.openAIProgramGenerator()">
    ...
    showProgramDetail(programId) {
        ...
        const totalDuration = program.exercises.reduce((sum, ex) => sum + ex.duration, 0);
        const totalMins = Math.floor(totalDuration / 60);
        const totalSecs = totalDuration % 60;
        const durationText = totalSecs > 0 ? `${totalMins}:${totalSecs.toString().padStart(2, '0')}` : `${totalMins} min`;
        ...
        document.getElementById('program-detail-duration').value = totalMins || program.duration || 12;
        ...
        program.exercises.forEach((exerciseRef, index) => {
            ...
            const exerciseName = exercise ? exercise.name : exerciseRef.exerciseId;
            const exerciseDuration = Math.floor(exerciseRef.duration / 60);
            const exerciseSecs = exerciseRef.duration % 60;
            const exerciseDurationText = exerciseSecs > 0 ? `${exerciseDuration}:${exerciseSecs.toString().padStart(2, '0')}` : `${exerciseDuration}m`;
    ...
    startProgramFromDetail() {
        ...
        // Start the program with custom duration
    ...
    scheduleAudioTrack(audioConfig, trackIndex) {
        ...
        const scheduler = setTimeout(() => {
            ...
            // Play for specified duration or until exercise ends
            audio.play().catch(err => console.log('Audio autoplay prevented:', err.message));
            
            // Stop after playDuration
            if (audioConfig.playDuration) {
                setTimeout(() => {
                    audio.pause();
                }, audioConfig.playDuration * 1000);
            }
    ...
    startLocalVideo(videoConfig) {
        ...
        if (videoContainer && videoElement) {
            ...
            // Stop after playDuration
            if (videoConfig.playDuration && videoConfig.playDuration < 99999) {
                setTimeout(() => {
                    videoElement.pause();
                }, (videoConfig.exerciseStart + videoConfig.playDuration) * 1000);
            }
    ...
    openProgramManager() {
        ...
        programsArray.forEach((program, index) => {
            ...
            programItem.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div style="flex: 1;">
                        <h4 style="color: #1a202c; margin-bottom: 0.5rem;">${program.name}</h4>
                        <p style="color: #4a5568; font-size: 0.875rem; margin-bottom: 0.5rem;">${program.description}</p>
                        <p style="color: #718096; font-size: 0.875rem;">${program.exercises.length} exercises • ${program.duration} min</p>
                    </div>
        ...
        document.getElementById('program-manager-modal').style.display = 'flex';
    ...
    editProgram(programId) {
        ...
        allExercises.forEach((exercise) => {
            ...
            item.innerHTML = `
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    ...
                    <input type="number" class="exercise-duration-edit" data-exercise-id="${exercise.id}" value="${Math.round(selectedDuration / 60)}" min="1" max="60" style="width: 60px; padding: 0.25rem; border: 1px solid #e2e8f0; border-radius: 4px;" placeholder="min">
                </label>
            `;
        ...
        document.getElementById('program-editor-modal').style.display = 'flex';
    }
    ...
    openExerciseManager() {
        ...
        document.getElementById('exercise-manager-modal').style.display = 'flex';
    }
    ...
    openProgramBuilder() {
        ...
        document.getElementById('program-builder-modal').style.display = 'flex';
    }
    ...
    openAIProgramGenerator() {
        ...
        document.getElementById('ai-program-modal').style.display = 'flex';
    }
    
    showSavedAIPrograms() {
        // Hide create section, show saved section
        document.getElementById('create-ai-program-section').style.display = 'none';
        document.getElementById('saved-ai-programs-section').style.display = 'block';
        ...
        if (Object.keys(savedPrograms).length === 0) {
            savedList.innerHTML = `
                <div style="text-align: center; padding: 2rem; background: #f7fafc; border-radius: 8px; border: 2px dashed #cbd5e0;">
                    <div style="font-size: 3rem; margin-bottom: 1rem;">🤖</div>
                    <p style="color: #4a5568; margin-bottom: 0.5rem; font-weight: 600;">No AI Programs Yet</p>
                    <p style="color: #718096; font-size: 0.875rem;">Click "Create New AI Program" below to generate your first personalized program!</p>
                </div>
            `;
        } else {
            savedList.innerHTML = Object.values(savedPrograms).map(program => `
                <div style="background: #f8f9fa; padding: 1rem; margin-bottom: 0.75rem; border-radius: 8px; border: 1px solid #e2e8f0; transition: all 0.2s;" 
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div style="flex: 1;">
                            <h4 style="color: #1a202c; margin-bottom: 0.5rem;">
                                ${program.name}
                                <span style="background: linear-gradient(135deg, #8b5cf6, #7c3aed); color: white; padding: 0.2rem 0.4rem; border-radius: 3px; font-size: 0.65rem; margin-left: 0.5rem;">🤖 AI</span>
                            </h4>
                            <p style="color: #4a5568; font-size: 0.875rem; margin-bottom: 0.5rem;">${program.description}</p>
                            <p style="color: #718096; font-size: 0.8rem;">${program.exercises.length} exercises • ${program.duration} min</p>
                        </div>
                        <div style="display: flex; gap: 0.5rem;">
    ...
    showCreateNewAIProgram() {
        ...
        document.getElementById('saved-ai-programs-section').style.display = 'none';
        document.getElementById('create-ai-program-section').style.display = 'block';
    ...
    // Start meditation program with bell system and ambient audio
    ...
    startProgramAudio(program) {
        // Start background ambient based on program type
        const ambientType = this.getAmbientSound(program.type, program.category);
        this.playBackgroundAmbient(ambientType, this.audioSystem.volume);
        ...
        // Start exercise sequence with bells
        this.executeExerciseSequence(program.exercises, 0);
    }
    
    executeExerciseSequence(exercises, currentIndex) {
        if (currentIndex >= exercises.length) {
            // All exercises completed - play continuous bell
            console.log('🔔 All exercises completed - playing continuous bell');
            this.playBell(0, 'continuous');
            
            // Stop background ambient after a delay
            setTimeout(() => {
                this.stopBackgroundAmbient();
            }, 5000);
            ...
            this.recordSession(this.selectedProgram, Math.round(totalDuration / 60), true);
        ...
        console.log(`🔔 Exercise ${exerciseNumber}/${exercises.length}: ${exercise.name}`);
        
        // Play bell sequence for current exercise
        this.playBell(exerciseNumber, 'single');
        
        // Show exercise notification
        this.showBellNotification(`🔔 Exercise ${exerciseNumber}: ${exercise.name}`);
        
        // Simulate exercise duration (in real implementation, this would be a timer)
        setTimeout(() => {
            console.log(`✅ Exercise ${exerciseNumber} completed`);
            this.executeExerciseSequence(exercises, currentIndex + 1);
        }, exercise.duration * 1000); // Convert to milliseconds
    }
    ...
    executeExerciseWithAudio(exercises, currentIndex) {
        if (currentIndex >= exercises.length) {
            // Program complete - stop all audio and play final bell
            this.stopAllAudio();
            this.playBell(0, 'continuous');
            this.showBellNotification('🎉 Program complete! Thank you for your practice.');
    ...
    startProgramFromDetail() {
        ...
        // Start the program
        this.startMeditationProgram(programId);
    }
    ...
    openExerciseManager() {
        document.getElementById('exercise-manager-modal').style.display = 'flex';
        this.renderExerciseManager();
    }
    
    // Close exercise manager
    closeExerciseManager() {
    ...
    // Render exercise manager content
    ...
    showExerciseDetails(exerciseId) {
        ...
        const details = `
            ...
            <p><strong>Safety Level:</strong> ${exercise.safetyLevel || 'beginner'}</p>
            <p><strong>Instructions:</strong> ${exercise.instructions || 'No instructions available'}</p>
            <p><strong>Benefits:</strong> ${exercise.benefits || 'No benefits listed'}</p>
