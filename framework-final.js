...
class WellnessFramework {
    ...
    startSlideshow(exerciseId) {
        const exercise = this.exercises[exerciseId];
        if (!exercise || !exercise.imageSlideshow) return;
    ...
    showNextSlide(slideshow, index) {
        ...
        if (img && info) {
            ...
            info.textContent = `${index + 1}/${slideshow.length} - ${mediaFile.name} (${slide.duration}s)`;
    ...
    stopSlideshow() {
        ...
        if (img) img.src = '';
        if (info) info.textContent = 'Slideshow stopped';
    }
